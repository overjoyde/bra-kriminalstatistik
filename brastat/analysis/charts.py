...
Kräver att dessa körts först:
    ...
    python scripts/domstat_watchlist.py                # (valfritt) för graf 08, domstolsledet och insolvens
...
from brastat import log as blog  # noqa: E402
...
DOMSTAT_SOURCE = "Källa: Domstolsverket (DOMstat), Brå (SOL). Bearbetning: bra-kriminalstatistik."
...
def load_domstat(d: Path) -> pd.DataFrame | None:
    """DOMstat-bevakningslistan (år) eller None om den inte hämtats."""
    p = d / "domstat" / "watchlist_samlad.csv"
    if not p.exists():
        return None
    df = pd.read_csv(p, sep=";", dtype={"dimension": str, "dimensionsvarde": str})
    df["ar"] = pd.to_numeric(df["ar"], errors="coerce")
    return df.dropna(subset=["ar"])


def domstat_series(dom: pd.DataFrame, serie: str, variabel: str, dimensionsvarde: str | None = None,
                   domstol: str = "Alla tingsrätter") -> pd.Series:
    x = dom[(dom.serie == serie) & (dom.variabel == variabel) & (dom.domstol == domstol)]
    if dimensionsvarde is not None:
        x = x[x.dimensionsvarde == dimensionsvarde]
    return x.groupby("ar").varde.sum(min_count=1).dropna().sort_index()


def annual_from_monthly(s: pd.Series) -> pd.Series:
    """Summera en månadsserie per år; bara år med alla tolv månader."""
    g = s.groupby(s.index.year)
    return g.sum()[g.count() == 12]
...
def chart_domstat(dom, sol, out):
    """Domstolsledet och insolvens: inkomna ärenden vid tingsrätt, med anmäld penningtvätt som jämförelse."""
    fig, (a1, a2) = plt.subplots(1, 2, figsize=(14, 4.4))
    for (serie, var, dim, label), c in zip(
            [("konkurser_tingsratt", "Antal inkomna konkursärenden", None, "Konkursärenden"),
             ("insolvensarenden_tingsratt", "Antal inkomna mål", "Företagsrekonstruktioner", "Företagsrekonstruktioner"),
             ("insolvensarenden_tingsratt", "Antal inkomna mål", "Skuldsanering", "Skuldsanering")], PALETTE):
        s = domstat_series(dom, serie, var, dim)
        if len(s):
            a1.plot(s.index, s.values, marker="o", ms=3, lw=2, color=c, label=label)
    a1.set_title("Inkomna insolvensärenden, alla tingsrätter")
    a1.legend(loc="upper left")
    thousands(a1)
    bm = domstat_series(dom, "brottmal_tingsratt", "Antal inkomna mål", "Brottmål")
    a2.plot(bm.index, bm.values, marker="o", ms=3, lw=2, color=BLUE, label="Inkomna brottmål, tingsrätt")
    a2.set_title("Brottmål i tingsrätt och anmäld penningtvätt")
    thousands(a2)
    pt = annual_from_monthly(monthly(sol, "penningtvatt_totalt")) if sol is not None else pd.Series(dtype=float)
    if len(pt):
        ax2 = a2.twinx()
        ax2.plot(pt.index, pt.values, marker="s", ms=3, lw=2, ls="--", color=ORANGE,
                 label="Anmälda penningtvättsbrott (höger axel)")
        ax2.grid(False)
        ax2.spines["right"].set_visible(True)
        thousands(ax2)
        h1, l1 = a2.get_legend_handles_labels()
        h2, l2 = ax2.get_legend_handles_labels()
        a2.legend(h1 + h2, l1 + l2, loc="upper left")
    else:
        a2.legend(loc="upper left")
    for ax in (a1, a2):
        ax.xaxis.set_major_locator(mtick.MaxNLocator(integer=True))
    save(fig, out, "08_domstat_insolvens_brottmal.png",
         DOMSTAT_SOURCE + " DOMstat saknar brottskoder – brottmål är alla brottstyper.")
...
def annual_table(sol, annual, out):
    ...
    for serie, r in a[years].iterrows():
        ...
        first = r.dropna()
        # Serier som saknas i datan (t.ex. om Brå bytt namn på en rad) ger en tom rad i stället för krasch.
        chg = f"{r.iloc[-1] / first.iloc[0] - 1:+.0%} ({first.index[0]}–{years[-1]})" if len(first) > 1 else ""
        lines.append(f"| {serie} | " + " | ".join(vals) + f" | {chg} |")
...
def main() -> None:
    ...
    dom = load_domstat(d)
    if dom is None:
        log.info("  (hoppar över DOMstat-grafen – kör domstat_watchlist.py först)")
    try:
        ...
        annual = susp = None
        log.warning("  (hoppar över årstabell/misstänkta – kör fetch_tables.py först: %s)", e)
    ...
    for name in (["light", "dark"] if a.theme == "both" else [a.theme]):
        ...
        with plt.rc_context(theme.rc(name)):
            ...
            if dom is not None:
                chart_domstat(dom, sol, out)
            if annual is not None:
                try:
                    chart_elderly(annual, out)
                    chart_suspects(susp, out)
                except (ValueError, KeyError, IndexError) as e:
                    log.warning("  (hoppar över äldre/misstänkta: %s)", e)
    if annual is not None:
